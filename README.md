...
**Live demo:** _add your deployed frontend URL here once deployed_
**API docs:** _add your deployed backend URL + `/docs` here (FastAPI auto-generates Swagger UI)_
...
- Create, edit, delete job applications (company, role, status, dates, notes)
...
| Database | PostgreSQL in production (SQLite for local dev — zero setup needed) |
...
3. Add a Postgres database on Render (or use Supabase) and set `DATABASE_URL`
   in the service's environment variables.
4. `JWT_SECRET` is auto-generated by the blueprint — leave as is.
